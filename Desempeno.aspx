...
<style>
        ...
        .nav-tabs .nav-link.active {
             font-weight: 600; /* Bootstrap 5 default is 500, making it a bit bolder */
             color: #F97316; /* Color naranja para la pestaña activa */
        ...
        .feedback-message { /* Para mensajes de feedback JS */
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
   ...
   <asp:UpdatePanel ID="UpdatePanelModal" runat="server" UpdateMode="Conditional">
        <ContentTemplate>
            ...
            <div class="card shadow-lg mx-auto" style="max-width: 60rem;"> <%-- Ancho máximo aumentado un poco --%>
                <div class="card-body p-4 p-md-5">
                    ...
                    <%-- SECCIÓN DE METAS (AHORA INTEGRADAS EN PESTAÑAS) --%>
                    <section id="metasSemanalesYVencidas" class="mb-4">
                        <h2 class="h4 mb-3 text-primary">Mis Metas</h2>
                        <nav>
                            <div class="nav nav-tabs mb-0" id="nav-tab" role="tablist">
                                <button class="nav-link" id="nav-overdue-tab" data-bs-toggle="tab" data-bs-target="#nav-overdue" type="button" role="tab" aria-controls="nav-overdue" aria-selected="false">Vencidas</button>
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            ...
                            <div class="tab-pane fade" id="nav-overdue" role="tabpanel" aria-labelledby="nav-overdue-tab" tabindex="0">
                                ...
                                <asp:Panel ID="pnlEmptyMetasVencidas" runat="server" Visible="false" CssClass="empty-data-panel">
                                    <div class="alert alert-secondary small mt-3">No hay metas vencidas pendientes.</div>
                                </asp:Panel>
                            </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ScriptsContent" runat="server">
    ...
    <script>
        ...
        const mainContentContainer = document.querySelector('.card-body');
        ...
        function initializeModal() {
            if (respuestaModalElement && typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                if (!respuestaModal) { // Initialize only if not already done
        ...
        function GetWeekOfMonthJS(date) {
            ...
            const dayOfWeekOfFirst = firstDayOfMonth.getDay() === 0 ? 7 : firstDayOfMonth.getDay(); 
        ...
        function setActiveWeekTab() {
            try {
                ...
                let activeTabValue = currentWeekNumberInMonth.toString();
                ...
                if (preSelectedTab) {
                    ...
                    if (preSelectedTab.id === 'nav-overdue-tab') {
                        activeTabValue = "overdue";
                    } else {
                ...
                if (hfActiveWeekNumber) {
                    hfActiveWeekNumber.value = activeTabValue;
                }
                ...
                if (targetTabButton && targetPane) {
                    ...
                    const firstAvailableTab = document.querySelector('#nav-tab .nav-link:not(.disabled)');
                    if(firstAvailableTab){
                        firstAvailableTab.classList.add('active');
                        firstAvailableTab.setAttribute('aria-selected', 'true');
                        const firstPaneId = firstAvailableTab.getAttribute('data-bs-target');
                        if(firstPaneId) document.querySelector(firstPaneId).classList.add('show', 'active');
                        if (hfActiveWeekNumber) {
                           hfActiveWeekNumber.value = firstAvailableTab.id === 'nav-overdue-tab' ? 'overdue' : (firstAvailableTab.dataset.weekCodeNum || "1");
                        }
                    }
                }
            } catch (error) {
        ...
        if (mainContentContainer) {
            mainContentContainer.addEventListener('click', function (event) {
                ...
                if (clickedListItem) {
                    ...
                    let codigoSemanaDeLaPestanaParaDetalles = null;
                    const activeTabButton = document.querySelector('#nav-tab .nav-link.active');
                    
                    if (activeTabButton) {
                        const tabId = activeTabButton.id;
                        if (tabId === 'nav-overdue-tab') {
                            codigoSemanaDeLaPestanaParaDetalles = "0000000"; 
                        } else {
                            const weekNumFromTab = activeTabButton.dataset.weekCodeNum;
                            if (weekNumFromTab) {
                                codigoSemanaDeLaPestanaParaDetalles = getCodigoSemanaServidorFormato(weekNumFromTab);
                            }
                        }
                    }
                    ...
                    if (metaId > 0) {
                        if (typeof PageMethods !== 'undefined' && PageMethods.GetRespuestaDetalles) {
                            PageMethods.GetRespuestaDetalles(metaId, esFinalizable, codigoSemanaDeLaPestanaParaDetalles,
                                (response) => {
            ...
            respuestaModalElement.addEventListener('click', function(event) {
                ...
                if (linkClickeado) {
                    ...
                    if (urlParaAbrir) {
                        ...
                        if (!nuevaVentana) alert('Bloqueador de pop-ups activado. Permita pop-ups para este sitio.');
                    } else {
                        alert('Error: URL del documento no encontrada.');
                    }
                }
            });
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            allTabButtons.forEach(tabButton => {
                tabButton.addEventListener('shown.bs.tab', function (event) {
                    ...
                    if (event.target.id === 'nav-overdue-tab') {
